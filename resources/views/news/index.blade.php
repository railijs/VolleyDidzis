            ...
            @foreach($news->skip(1) as $item)
                <div class="flex flex-col bg-white shadow-md hover:shadow-xl transition overflow-hidden rounded-lg">
                    ...
                    <div class="flex flex-col flex-1 p-4 sm:p-6">
                        @if($item->category)
                            <span class="text-xs sm:text-sm uppercase tracking-wide text-indigo-600 font-semibold mb-1 sm:mb-2">
                                {{ $item->category }}
                            </span>
                        @endif
...
<footer class="bg-gray-100 dark:bg-gray-900 py-6 sm:py-8">
    <div class="max-w-6xl mx-auto px-4 sm:px-6 text-center">
        ...
        <p class="mt-4 sm:mt-6 text-xs sm:text-sm text-gray-500 dark:text-gray-400">&copy; {{ date('Y') }} VolleyLV. All rights reserved.</p>
    </div>
</footer>
