<x-app-layout>
    <div class="relative min-h-screen pt-24 pb-16 bg-gradient-to-b from-white via-red-50 to-white">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            @php
                $q = trim(request('q', ''));
                $totalNews = method_exists($news, 'total') ? $news->total() : $news->count();
                $categories = collect($news)->pluck('category')->filter()->unique()->take(10);
            @endphp
            ...
            <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-6 fade-up">
                <div>
                    ...
                    @if ($q !== '')
                        <p class="mt-1 text-sm text-gray-600">Meklēts pēc: <span
                                class="font-semibold">“{{ $q }}”</span> • Atrastas: {{ $totalNews }}</p>
                    @endif
                </div>
            ...
            <div class="fade-up">
                <form method="GET" action="{{ route('news.index') }}" class="relative group">
                    <input type="search" name="q" value="{{ $q }}"
                        placeholder="Meklēt ziņas, kategorijas vai atslēgvārdus…"
                ...
                @if ($categories->isNotEmpty())
                    <div class="mt-3 flex flex-wrap gap-2">
                        @foreach ($categories as $cat)
                            <a href="{{ route('news.index', array_filter(['q' => $cat])) }}"
                                class="px-3 py-1 rounded-full text-xs font-semibold border border-gray-200 text-gray-700 bg-white hover:bg-red-50 hover:border-red-200 hover:text-red-700 transition">
                                # {{ $cat }}
                            </a>
                        @endforeach
                        @if ($q !== '')
                            <a href="{{ route('news.index') }}"
                                class="px-3 py-1 rounded-full text-xs font-semibold bg-gray-100 text-gray-600 hover:bg-gray-200 transition">
                                Notīrīt filtrus
                            </a>
                        @endif
                    </div>
                ...
                <section class="relative overflow-hidden rounded-2xl shadow-xl mt-8 mb-8 group fade-up">
                    ...
                    <div class="relative z-10 p-5 sm:p-8 max-w-3xl">
                        @if ($featured->category)
                            <span class="text-[11px] tracking-[0.18em] text-red-300/90 font-bold uppercase">
                                {{ $featured->category }}
                            </span>
                        @endif
                    ...
                    @foreach ($news->skip(1) as $item)
                        <article
                            ...
                            <div class="p-4 sm:p-5 flex flex-col h-full">
                                @if ($item->category)
                                    <span class="text-[10px] uppercase tracking-widest text-red-600 font-bold mb-1.5">
                                        {{ $item->category }}
                                    </span>
                                @endif

                                <h3 class="text-base sm:text-lg font-semibold text-gray-900 mb-1 line-clamp-2">
                                    <a href="{{ route('news.show', $item) }}" class="hover:text-red-600 transition">
                                        {{ $item->title }}
                                    </a>
                                </h3>
                                ...
                                <div class="flex items-center justify-between">
                                    ...
                                    @if ($item->category)
                                        <span
                                            class="px-2 py-0.5 rounded-full bg-red-50 text-red-700 text-[10px] font-semibold">
                                            {{ $item->category }}
                                        </span>
                                    @endif
                                </div>
            ...
            <div class="mt-8 fade-up">
                {{-- saglabā meklējuma frāzi lapošanā --}}
                @if (method_exists($news, 'appends'))
